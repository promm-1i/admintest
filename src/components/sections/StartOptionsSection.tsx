...
const TEMPLATE_INDUSTRIES: TemplateIndustry[] = [
  ...
  { key: "moto", name: "오토바이 · 스쿠터", icon: Bike, img: "/thumbs/moto-a.jpg", href: "/templates?industry=moto", title: "오토바이 · 스쿠터 홈페이지", note: "제품 상세와 구매 · 리스 상담, A/S 절차와 협력점 지도로 문의를 만드는 구성입니다.", points: ["제품 상세 · 사양", "구매 · 리스 상담", "A/S 절차", "협력점 지도"] },
]
  // 가나다순으로 보여주고, 미리보기 사진은 그 업종에서 가장 최근에 추가된 랜딩형 시안(SAMPLES 앞쪽)으로 바꾼다
  .map((t) => ({
    ...t,
    img: SAMPLES.find((s) => s.industryKey === t.key && s.type.includes("landing-template"))?.image ?? t.img,
  }))
