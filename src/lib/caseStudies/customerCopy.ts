...
const POINT_OVERRIDES: Record<string, PointOverride> = {
  ...
  "작품 · 격자": { subject: "작품 포트폴리오", title: "작품을\n크기가 다른 격자로 둘러봅니다" },
  ...
  "홈 · 사업 장면": { subject: "주요 사업", title: "주요 사업을\n카드로 나란히 놓았습니다" },
  ...
  "홈 · 파트너": { subject: "글로벌 파트너", title: "협력사 로고를\n한 줄로 흘려 놓았습니다" },
  ...
  "회사개요 · 사업영역": { subject: "사업영역", title: "사업영역은 분야별로\n상세 쪽으로 이어집니다" },
  ...
  인재채용: { subject: "인재채용", title: "인재상과 채용 공고를\n한 쪽에 모았습니다" },
  ...
  "IR 개요 · 신용등급과 보고서": { subject: "신용등급·경영보고서", title: "경영지표 · 신용등급 · 보고서를\n따로 둡니다" },
  ...
  "메인 · 담당 중개사": { subject: "담당 중개사", title: "담당자 얼굴과 경력을\n상담 버튼 옆에 둡니다" },
  ...
  "홈 · 블로그 · 후기": { subject: "블로그·고객 후기", title: "후기와 블로그 글을\n상담 버튼 앞에 둡니다" },
...
const BENEFITS: { pattern: RegExp; copy: BenefitCopy }[] = [
  ...
  },
  {
    ...
    copy: {
      ...
      items: ["주요 활동과 성과를 연도별로 정리", "현장 사진으로 진정성 전달", "지속가능경영 정보와 연결"],
    },
  },
  {
    ...
    copy: {
      title: () => "주요 개발 단지\n실제 사업 사례로 보여줍니다",
      body: () =>
        "대표 개발 단지의 사진과 핵심 정보를 한 카드에 붙여, 어떤 규모의 일을 해 왔는지 한 번에 보이게 합니다.",
  ...
  },
  {
    ...
    copy: {
      title: () => "윤리경영 원칙\n명확하게 안내합니다",
      body: () =>
        "윤리 기준과 실천 원칙을 조항별로 나눠, 필요한 항목만 펼쳐 읽게 합니다.",
  ...
  },
  {
    ...
    copy: {
      ...
      items: ["주요 연구 분야와 성과 소개", "제품 개발과의 연관성 안내", "기술·인증 자료로 바로 이동"],
    },
  },
  {
  ...
  },
  {
    ...
    copy: {
      title: (subject) => `${subject}\n작품 중심으로 보여줍니다`,
      body: (subject) =>
        `${subject} 내용을 큰 사진과 짧은 설명으로 놓아, 글보다 작품이 먼저 보이게 합니다.`,
      items: ["작품 이미지를 선명하고 크게 소개", "작가 의도와 작품 정보를 함께 제공", "전시 일정과 문의로 바로 이동"],
    },
  },
  {
  ...
  },
  {
    ...
    copy: {
      ...
      items: ["고객 후기와 실제 성과 강조", "후기와 함께 근거 자료 제공", "실제 브랜드 자료에 맞게 변경 가능"],
    },
  },
  {
  ...
  },
  {
    ...
    copy: {
      ...
      items: ["핵심 기술과 장점을 간결하게 설명", "사진·도표를 활용한 이해도 향상", "인증·성과 자료로 바로 이동"],
    },
  },
  {
  ...
  },
  {
    ...
    copy: {
      ...
      items: ["대표 사례와 결과물을 시각적으로 강조", "프로젝트별 핵심 정보 제공", "상세 화면과 상담으로 바로 이동"],
    },
  },
  {
    ...
    copy: {
      title: (subject) => `${subject}\n브랜드 방향성을 전달합니다`,
      body: (subject) =>
        `${subject} 내용을 사진과 짧은 문장으로 번갈아 놓아, 회사 소개를 긴 문단으로 읽지 않게 합니다.`,
  ...
  },
  {
    ...
    copy: {
      title: (subject) => `${subject}\n한눈에 비교합니다`,
      body: (subject) =>
        `${subject} 영역의 서비스와 상품을 같은 모양 카드로 나란히 놓아 견주게 하고, 카드를 누르면 상세로 넘어갑니다.`,
...
function sectionCopy(name: string, source = ""): { desc: string; items: [string, string, string] } {
  ...
  if (/홈|메인|첫 화면/.test(name)) {
    return {
      ...
      items: ["대표 서비스와 브랜드 강점 강조", "주요 콘텐츠로 빠르게 이동", "문의 화면으로 바로 이동"],
    };
  }
  ...
  if (/소개|개요|비전|연혁|철학|회사/.test(text)) {
    return {
      desc: "회사 소개와 걸어온 과정을 연도 순서로 놓았습니다.",
  ...
  return {
    ...
    items: ["핵심 정보 중심의 간결한 구성", "브랜드 자료에 맞춘 내용 변경", "관련 화면으로 바로 이동"],
  };
}
...
function customerProfile(industry: string): {
  ...
  if (/작가|아티스트/.test(industry)) {
    return {
      journey: "작품을 보다가 전시 일정과 문의로 넘어갑니다",
  ...
  if (/부동산/.test(industry)) {
    return {
      journey: "매물을 찾고 상세를 본 뒤 상담을 넣습니다",
...
export function toCustomerFacingCaseStudy(study: CaseStudy): CaseStudy {
  ...
  return {
    ...study,
    summary: `${industry} 홈페이지에 필요한 화면을 갖춘 프리미엄 디자인입니다. ${sections} 화면이 들어가고, ${profile.journey}.`,
    overview: `${study.brand} 디자인은 ${industry} 분야를 가정해 구성했습니다. 소개 → 서비스 → 근거 자료 → 문의 순서로 놓아, 처음 온 사람도 위에서부터 읽으면 되게 했습니다.\n\n${multiPage ? "내용이 많은 쪽은 목적별로 화면을 나눠 필요한 것만 찾아 봅니다." : "한 페이지 안에서 소개부터 서비스, 근거 자료, 문의까지 위에서 아래로 이어집니다."} 회사명, 색상, 사진, 메뉴, 문구는 실제 브랜드와 운영 방식에 맞게 변경하며 PC·태블릿·모바일에서 편리하게 이용할 수 있도록 제작합니다.\n\n화면에 표시된 회사명, 인물, 작품, 상품, 가격, 실적 등은 디자인 구성을 보여 주기 위한 예시입니다.`,
    meta: [
    ...
    mobile: {
      ...
      body: `작은 화면에서는 한 줄씩 쌓여, 핵심 정보를 읽고 ${profile.inquiryLabel}까지 그대로 내려갑니다.`,
