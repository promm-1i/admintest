...
import { useMovingAdmin } from "./moving-admin/store";
import { Modal } from "./real-estate-admin/components";
import type { Case } from "./moving-admin/types";

export default function MovingCustomerSitePage() {
  usePageTitle(
    "NOVERIQ 이사·청소 (데모)",
    "NOVERIQ 이사·청소업체 맞춤형 홈페이지 웹 솔루션으로 구축한 고객용 홈페이지 데모입니다.",
  );
          ...
          <div>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setSelected(null);
                setTarget(c);
                setMemo(`${c.name}과(와) 비슷한 조건 견적 문의드립니다.`);
