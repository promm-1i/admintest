...
import type { Department } from "./hospital-admin/types";

export default function HospitalCustomerSitePage() {
  usePageTitle(
    "MintCL 의원 (데모)",
    "MintCL 병원·의원 맞춤형 홈페이지 웹 솔루션으로 구축한 고객용 홈페이지 데모입니다.",
  );

  const { departments, nonCovered, setInquiries, logActivity } = useHospitalAdmin();
  ...
  const publicDepartments = departments.filter((d) => d.status === "공개");
  ...
  const submit = (e: FormEvent) => {
    ...
    setInquiries((prev) => [
      { id: Date.now(), name: name.trim(), phone: phone.trim(), department: target?.name ?? "미지정", content: memo.trim() || "예약 요청", status: "접수" },
      ...prev,
    ]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex items-center justify-center gap-1.5 bg-foreground px-4 py-1.5 text-center text-[11px] text-background">
        <span>이 화면은 관리자 페이지에서 등록·공개한 진료과목이 실시간 반영되는 고객용 홈페이지 데모입니다.</span>
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">MintCL 의원</p>
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected?.name ?? ""}>
        {selected && (
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-foreground break-keep">{selected.desc}</p>
            <Button
