...
export default function AcademyCustomerSitePage() {
  ...
  const { courses, reviews, setInquiries, logActivity } = useAcademyAdmin();
  ...
  const submit = (e: FormEvent) => {
    ...
    setInquiries((prev) => [
      { id: Date.now(), name: name.trim(), phone: phone.trim(), course: target?.name ?? "미지정", content: memo.trim() || "상담 요청", status: "접수" },
      ...prev,
    ]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex items-center justify-center gap-1.5 bg-foreground px-4 py-1.5 text-center text-[11px] text-background">
        <span>이 화면은 관리자 페이지에서 등록·공개한 강의가 실시간 반영되는 고객용 홈페이지 데모입니다.</span>
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">MintCL 학원</p>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12">
        ...
        {publishedReviews.length > 0 && (
          <div className="mt-14">
            <h2 className="text-lg font-semibold text-foreground">합격 · 후기</h2>
            <div className="mt-4 space-y-2">
              {publishedReviews.map((r) => (
                <div key={r.id} className="rounded-lg border border-border bg-secondary/30 px-4 py-3 text-sm text-foreground">{r.title}</div>
              ))}
            </div>
          </div>
        )}
      ...
      <section id="contact" className="border-t border-border bg-foreground px-4 py-10 text-background">
        <div className="mx-auto grid max-w-4xl gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            ...
            <a href="tel:02-000-0000" className="mt-5 inline-flex items-center gap-2 text-lg font-bold">
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={selected?.name ?? ""}>
        {selected && (
          <div>
            <p className="text-sm text-muted-foreground">{selected.subject} · {selected.schedule}</p>
