...
export default function InteriorCustomerSitePage() {
  ...
  const { cases, packages, setInquiries, logActivity } = useInteriorAdmin();
  ...
  const submit = (e: FormEvent) => {
    ...
    setInquiries((prev) => [
      { id: Date.now(), name: name.trim(), phone: phone.trim(), area: target?.area ?? "미지정", content: memo.trim() || "견적 요청", status: "접수" },
      ...prev,
    ]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex items-center justify-center gap-1.5 bg-foreground px-4 py-1.5 text-center text-[11px] text-background">
        <span>이 화면은 관리자 페이지에서 등록·공개한 시공 사례가 실시간 반영되는 고객용 홈페이지 데모입니다.</span>
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">MintCL 인테리어</p>
          ...
          <div className="mt-14">
            ...
            <div className="mt-4 space-y-2">
              {publishedPackages.map((p) => (
                <div key={p.id} className="rounded-lg border border-border bg-secondary/30 px-4 py-3 text-sm text-foreground">{p.title}</div>
              ))}
            </div>
          </div>
        )}
      ...
      <footer className="border-t border-border bg-secondary/30 px-4 py-8 text-center text-xs text-muted-foreground">
        <p>MintCL 인테리어 (데모) · 02-000-0000</p>
