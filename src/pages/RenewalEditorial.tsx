...
function Header() {
  ...
  return <header className={`header re-header${mobileOpen ? " active" : ""}`} onPointerLeave={() => setMenuOpen(false)}>
    <div className="header-frame">
      ...
      <div className="header-nav"><div className="header-gnb"><nav className="gnb re-header__desktop" data-open={menuOpen} aria-label="주요 메뉴"><span className="re-mega__backdrop" aria-hidden="true" />{HEADER_NAV.map((entry) => entry.type === "link" ? <Link className="gnb-1d-link" key={entry.key} to={previewHref(entry.href)}>{entry.label}</Link> : <div className="gnb-1d-item re-nav-group" key={entry.key} data-open={menuOpen} onPointerEnter={() => setMenuOpen(true)} onFocusCapture={() => setMenuOpen(true)}><Link className="gnb-1d-link" to={previewHref(firstHref(entry))} aria-haspopup="true" aria-expanded={menuOpen} onKeyDown={(event) => { if (event.key === "Escape") { setMenuOpen(false); event.currentTarget.blur(); } }}>{entry.label}<ChevronDown /></Link><div className="re-mega"><div className="re-mega__links">{entry.items.map((item) => <div className="re-mega__col" key={item.href}><Link className="re-mega__lead" to={previewHref(item.href)}>{item.label}<ArrowUpRight /></Link>{item.children && item.children.filter((group) => group.designs.length > 0).slice(0, 6).map((group) => <Link className="re-mega__sub" key={group.key} to={previewHref(group.href)}>{group.label}<small>{group.designs.length}</small></Link>)}</div>)}</div></div></div>)}</nav></div></div>
