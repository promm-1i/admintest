    ...
    >
      <div className="mx-auto max-w-6xl px-5 h-[72px] flex items-center justify-between">
        ...
        >
          ...
          <button className="lg:hidden p-2 -mr-2 text-inkg" aria-label="메뉴" aria-expanded={open} onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="lg:hidden border-t border-inkg/15 bg-cream px-5 py-2">
          {SITE.nav.map((n) => (
            <button
              key={n.href}
              onClick={() => { setOpen(false); goTo(n.href) }}
              className="block w-full text-left py-3.5 text-[1rem] font-semibold text-inkg border-b border-inkg/10 last:border-0"
            >
              {n.label}
            </button>
          ))}
