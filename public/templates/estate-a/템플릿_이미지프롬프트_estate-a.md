...
2026-09-07 사용자 생성 사진 29장 설치 완료 — 용량을 위해 전부 `.webp` 로 변환(JPG q82 · 알파 PNG q88). 표의 파일명은 원본 기준, 실제 파일은 같은 이름의 `.webp`. `highlight.mp4` 는 사용자 영상(바탕화면 1.mp4, 1280×720 8초)에서 앞 2초(엉뚱한 웹사이트 컷)를 잘라내고 우하단 워터마크를 피해 1044×720 으로 크롭 → 1440×992 스케일 → 정방향+역방향 팰린드롬 11.75초, crf 24, 무음으로 설치했다. 폰용 축소본은 `*-m.webp`(16장, srcset).
