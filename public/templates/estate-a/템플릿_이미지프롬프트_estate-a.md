...
2026-09-07 사용자 생성 사진 29장 설치 완료 — 용량을 위해 전부 `.webp` 로 변환(JPG q82 · 알파 PNG q88). 표의 파일명은 원본 기준, 실제 파일은 같은 이름의 `.webp`. `highlight.mp4` 만 아직 포스터로 만든 임시 4초 클립이다(영상 프롬프트: 바탕화면 `estate-a_영상프롬프트.md`).
