(function (global, factory) {
  ...
  */
  ...
  var _clampScrollAndGetDurationMultiplier = function _clampScrollAndGetDurationMultiplier(scrollFunc, current, end, max) {
    ...
    node && node.nodeType !== 1 && (node = node.parentElement || node.parentNode);
    node || (node = _body$1);
    cache = node._gsap || gsap$1.core.getCache(node);

    if (!cache._isScrollT || time - cache._isScrollT > 2000) {
      while (node && node !== _body$1 && (node.nodeType !== 1 || node.scrollHeight <= node.clientHeight && node.scrollWidth <= node.clientWidth || !(_overflow[(cs = _getComputedStyle(node)).overflowY] || _overflow[cs.overflowX]))) {
        node = node.parentElement || _body$1;
      }
