...
### 2026-09-21 3차 — 의도한 차이 추가

**연혁 자리 제거.** 원본은 이야기 카드에 실제 연도(1899·1968·1977)를 넣는다. NOVERIQ 는 쓸 연혁이 없어 사용자 결정으로 그 자리를 뺐다. 그 결과 390 폭에서 카드 높이가 원본보다 49px 짧다(378 → 330). 1440 과 768 은 영향 없다. `audit_woori_parity.mjs` 의 `firstYear` 쌍은 비교에서 제외했다.

**견적에서 호스팅 항목 제거.** 호스팅을 따로 받지 않기로 해 항목을 없애고, 첫 해 호스팅료 240,000원을 네 항목에 60,000원씩 균등하게 녹였다. 총액은 그대로다. 기본형·원페이지 640,000원, 랜딩형·서브페이지 1,140,000원.

**본 사이트 컴포넌트에 리뉴얼 껍데기.** 업종 솔루션 9쪽과 업종 랜딩은 본 사이트 컴포넌트를 그대로 쓰고 리뉴얼 헤더·푸터·상단 제목 안에서 그린다. 안쪽 화면은 본 사이트 디자인 그대로다. 중첩 `Routes` 는 상위가 `/renewal-editorial/*` 라 상대 경로로 써야 한다.
