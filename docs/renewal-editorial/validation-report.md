...
`noveriq.co.kr` 의 자동 배포는 사용자가 일부러 멈춰 두었다. 최근 수정이 많아 한꺼번에 반영하려는 것이다. 라이브 사이트가 커밋보다 뒤쳐져 있어도 파이프라인 고장이 아니다. 커밋과 푸시는 평소대로 진행하고, 배포를 다시 켤 시점은 사용자가 정한다.

`/renewal-editorial` 은 기존 사이트를 덮어쓰지 않는 별도 시안 경로다. 본 사이트로 바꿔 끼우는 것은 별도 결정 사항이다.
